			...
			if (isInfoBufferEmpty()) {
				...
				if (buffer != NULL) {
					...
					if (new_pad & PAD_UP) {
						...
						/* Back: up one level. Previously CIRCLE did nothing in a
						 * menu -- the only way out of a submenu was its first
						 * entry -- even though the button bar advertises it. At
						 * the root there is no parent, so this does nothing. */
						...
						/* Options: jump straight to the Advanced Menu from
						 * anywhere, rather than walking down to it. Doing
						 * nothing when already there keeps it idempotent. */
						Menu *advanced = getAdvancedMenu();

						if ((advanced != NULL) && (advanced != menu)) {
							setCurrentMenu(advanced);
						}
					}
				} else {

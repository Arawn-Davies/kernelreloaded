...
moduleEntry_t modules[] = {
	...
	{
		/* Slim PSTwo ethernet, driven from the IOP.
		 *
		 * Restored after removing it broke NFS root on a slim. On that
		 * hardware Linux cannot drive the adapter from the EE side at all --
		 * see the comment on intrelay-dev9-rpc below: "Network not working
		 * from EE side (use smaprpc.irx)". USB and HDD work without it, which
		 * is why a ramdisk boot was unaffected and only NFS failed.
		 *
		 * Now embedded in the ELF rather than fetched over host:, like
		 * intrelay. */
	 ...
	 *
	 * Neither exists any more: ps2smap was renamed and restructured in ps2sdk
	 * (the SMAP driver is now smap*.irx with different arguments), and ps2link
	 * moved out to its own repository. smaprpc was recovered from git history
	 * and is restored above -- removing it was wrong.
	 *

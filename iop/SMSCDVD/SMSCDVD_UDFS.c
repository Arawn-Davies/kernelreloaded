...
static void UDFICB(unsigned char *apData, unsigned char *apFileType,
	unsigned short *apFlags)
{
...
static void UDFExtentAD(unsigned char *apData, unsigned int *apLen,
	unsigned int *apLoc)
{
...
static void UDFPartition(unsigned char *apData, unsigned short *apFlags,
	unsigned short *apNb, char *apContents, unsigned int *apStart,
	unsigned int *apLen)
{
	...
	do {
		...
		do {
			...
			if (!ReadDVDVSectors(lLBA++, 1, s_Buffer))
				...
				UDFDescriptor(lpLB, &lTagID);

			if (lTagID == 5 && !s_PartInfo.m_fValid) {

				UDFPartition(lpLB, &s_PartInfo.m_Flags,
					&s_PartInfo.m_Number,
					s_PartInfo.m_Contents,
					&s_PartInfo.m_Start, &s_PartInfo.m_Length);
...
/* wrong sector size */
			...
			(lTagID != 8) && (!s_PartInfo.m_fValid || !lfVolValid)
			);
...
static void UDFAD(unsigned char *apData, UDFAddress * apAddr,
	unsigned char aType)
{
...
static int UDFMapICB(UDFAddress * apICB, UDFAddress * apFile)
{
	...
	do {
		...
		/* end if */
			...
			s_PartInfo.m_Start + apICB->m_Location + (apICB->m_Length -
				1) / 2048) && lTagID != 261);
...
static int UDFFileIdentifier(unsigned char *apData, unsigned char *apFileChar,
...
static int UDFScanDir(UDFAddress * apScanDir, char *apFileName,
	UDFAddress * apFileICB)
{
	...
	if (ReadDVDVSectors(lScanLBA, 2, s_Buffer) <= 0)
		return 0;
	...
	while (lScanPos < lScanLen) {
		...
		if (lScanTagID == 257) {
			...
			lScanPos +=
				UDFFileIdentifier(&s_ScanBuf[lScanPos], &lScanFileChar,
				s_ScanTemp, apFileICB);

			if (!strcasecmp(apFileName, s_ScanTemp))
				return 1;
...
static int UDFInit(void)
{
	...
	if (UDFFindPartition(0)) {
		...
		do {
			...
			if (lTagID == 256)
				UDFAD(lpLB + 400, &s_RootICB, UDFADlong);
		...
		if (lTagID == 256 &&	/* good descriptor */
			...
			UDFMapICB(			/* root dir found  */
				&s_RootICB, &s_RootDir) && s_RootDir.m_Type == 4)
			retVal = 1;
...
static int UDF_FindFile(const char *apPath, UDFAddress * apFile)
{
	...
	*apFile = s_RootDir;
	...
	if (s_DirName[lLen - 1] == '/' || s_DirName[lLen - 1] == '\\')
	...
	if (lpPos) {
		...
		while (lpPos) {
			...
			if (!UDFScanDir(apFile, lpPos, &lICB))
				return 0;
			if (!UDFMapICB(&lICB, apFile))
				return 0;
...
static int UDF_DOpen(iop_io_file_t * apFile, const char *apPath)
{
	...
	if (s_ScanPos != 0xFFFFFFFF)
		return -ENFILE;

	lDir = s_RootDir;

	if (!UDF_FindFile(apPath, &lDir) || lDir.m_Type != UDF_FILE_TYPE_DIRECTORY)
		return -ENOENT;
...
static int UDF_DRead(iop_io_file_t * apFile, void *apRetVal)
{
	...
	while (s_ScanPos < s_ScanLen) {
		...
		if (s_ScanPos >= 2048) {
			...
			if (ReadDVDVSectors(s_ScanLBA++, 2, s_Buffer) <= 0)
				return 0;
		...
		if (s_ScanTagID == 257) {
			...
			s_ScanPos +=
				UDFFileIdentifier(&s_ScanBuf[s_ScanPos], &s_ScanFileChar,
				s_ScanTemp, &lFileICB);
...
static int UDF_Open(iop_io_file_t * apFile, const char *apName)
{
	...
	if (!UDF_FindFile(apName, &lFile) || lFile.m_Type != UDF_FILE_TYPE_FILE)
		return -ENOENT;
...
static int UDF_Read(iop_io_file_t * apFile, void *apBuff, int aSize)
{
	...
	if (lStartSector == s_LastSector) {
		...
		if (s_LastBk > 0)
			mips_memcpy(s_Buffer, s_Buffer + 2064 * s_LastBk, 2064);
	...
	if (lfRead == 0 || (lfRead == 1 && lnSectors > 1)) {
		...
		ReadDVDVSectors(lStartSector + lfRead, lnSectors - lfRead,
			s_Buffer + lnExtra);
	...
	if (i > 0) {
		...
		if (lOffSector)
			mips_memcpy(apBuff, lpBuff + 12, lOffSector);

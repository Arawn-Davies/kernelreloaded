...
config option already set in the repo's `linux/kernelconfig`, switched on at boot by
...
`linux/buildlinux.sh` already encodes the whole recipe — fetch a
...
Patches divide into the three `linux/buildlinux.sh` already applies, the two we need,
...
`linux/kernelconfig` — 864 lines, generated by `make menuconfig` against this tree —
...
They ship in this repo, as `linux/driver_ps2fs/ps2fs` and `linux/driver_unionfs/unionfs`,
...
| `No rule to make target 'smaprpc.o'` | `CONFIG_PS2_ETHER_SMAP=y` needs `linux/driver_slim_smaprpc/` copied in |
...
Two kernel changes were needed and are in `linux/phase1/patches/romcons-input.patch`: the
